...
std::string adb_basename(const std::string& path) {
  ...
  char ext[_MAX_EXT];
  _splitpath(&result[0], drive, dir, fname, ext);
  std::string tmpstr = fname;
  tmpstr += ext;
  char* name = (char*)tmpstr.c_str();
#endif
...
std::string adb_dirname(const std::string& path) {
  ...
  char ext[_MAX_EXT];
  _splitpath(&result[0], drive, dir, fname, ext);
  std::string tmpstr = drive;
  tmpstr += dir;
  char* parent = (char*)tmpstr.c_str();
#endif
...
std::string dump_hex(const void* data, size_t byte_count) {
    byte_count = min(byte_count, size_t(16));
