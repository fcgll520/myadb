...
static void send_window_size_change(int fd, std::unique_ptr<ShellProtocol>& shell) {
    ...
    size_t l = _snprintf(shell->data(), shell->data_capacity(), "%dx%d,%dx%d",
...
int adb_commandline(int argc, const char** argv) {
    ...
    if (!server_socket_str) {
		...
		char temp[200] = {0};
        if (server_host_str) {
            rc = sprintf(temp, "tcp:%s:%d", server_host_str, server_port);
        } else {
			rc = sprintf(temp, "tcp:%d", server_port);
        }
        if (rc < 0) {
