...
//    const DWORD module_result = GetModuleFileNameA(NULL, program_path,
...
//
//    char   args[64];
//    //_snprintf(args, arraysize(args), "adb -L %s fork-server server --reply-fd %d",
//    //          socket_spec.c_str(), ack_write_as_int);
//	_snprintf(args, arraysize(args), "adb -L %s fork-server server",
//		socket_spec.c_str());
//
...
int adb_server_main(int is_daemon, const std::string& socket_spec, int ack_reply_fd)
{
	...
	std::string local_name =
		android::base::StringPrintf("%s", socket_spec.c_str());
...
int main(int argc, char **argv)
{
	...
	return adb_commandline(argc - 1, (const char**)(argv + 1));
#else
