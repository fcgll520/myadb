...
bool WriteStringToFile(const std::string& content, const std::string& path,
  ...
  int flags = O_WRONLY | O_CREAT | O_TRUNC | O_CLOEXEC | O_BINARY |
              (follow_symlinks ? 0 : O_NOFOLLOW);
  int fd = 0;// TEMP_FAILURE_RETRY(open(path.c_str(), flags, DEFFILEMODE));

  {
	  int _rc;
	  do
	  {
		  _rc = open(path.c_str(), flags, DEFFILEMODE);
	  } while (_rc == -1 && errno == EINTR);
	  fd = _rc;
  }
...
bool ReadFully(int fd, void* data, size_t byte_count) {
  ...
  while (remaining > 0) {
	  size_t n = 0;// TEMP_FAILURE_RETRY(read(fd, p, remaining));

	{
		size_t _rc;
		do
		{
			_rc = read(fd, p, remaining);
		} while (_rc == -1 && errno == EINTR);
		n = _rc;
	}
...
bool WriteFully(int fd, const void* data, size_t byte_count) {
  ...
  size_t remaining = byte_count;
  while (remaining > 0) {
	  size_t n = 0;// TEMP_FAILURE_RETRY(write(fd, p, remaining));

	{
		size_t _rc;
		do
		{
			_rc = write(fd, p, remaining);
		} while (_rc == -1 && errno == EINTR);
		n = _rc;
	}
