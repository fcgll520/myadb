...
extern int adb_poll(adb_pollfd* fds, size_t nfds, int timeout) {
    ...
    for (size_t i = 0; i < nfds; ++i) {
        ...
        if (!fh || !fh->used || fh->clazz != &_fh_socket_class) {
            ...
            WSAPOLLFD wsapollfd = {
                fh->u.socket,
                static_cast<short>(fds[i].events)
            };
...
int network_loopback_client(int port, int type, std::string* error) {
    ...
    _snprintf(f->name, sizeof(f->name), "%d(lo-client:%s%d)", fd, type != SOCK_STREAM ? "udp:" : "",
             port);
...
static int _network_server(int port, int type, u_long interface_address, std::string* error) {
    ...
    const int fd = _fh_to_int(f.get());
	_snprintf(f->name, sizeof(f->name), "%d(%s-server:%s%d)", fd,
...
// with GetProcAddress("GetAddrInfoW").
    ...
    const int fd = _fh_to_int(f.get());
	_snprintf(f->name, sizeof(f->name), "%d(net-client:%s%d)", fd, type != SOCK_STREAM ? "udp:" : "",
             port);
...
int  adb_socket_accept(int  serverfd, struct sockaddr*  addr, socklen_t  *addrlen)
{
	...
	_snprintf(fh->name, sizeof(fh->name), "%d(accept:%s)", fd, serverfh->name);
...
static int _console_read(const HANDLE console, void* buf, size_t len) {
    for (;;) {
        ...
        if (!g_console_input_buffer.empty()) {
            const int bytes_read = min(len, g_console_input_buffer.size());
...
//    }
//
//	static_assert(sizeof(struct utimbuf) == sizeof(struct utimbuf),
