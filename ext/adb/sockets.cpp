...
static asocket local_socket_list = {
    &local_socket_list, &local_socket_list,
};
...
static asocket local_socket_closing_list = {
    &local_socket_closing_list, &local_socket_closing_list,
};
...
size_t asocket::get_max_payload() const {
    ...
    if (transport) {
        max_payload = min(max_payload, transport->get_max_payload());
    }
    if (peer && peer->transport) {
        max_payload = min(max_payload, peer->transport->get_max_payload());
    }
    return max_payload;
}
