...
static bool should_pull_file(int mode) {
    return mode & (S_IFREG | S_IFBLK | S_IFCHR);
}

static bool should_push_file(int mode) {
    int mask = S_IFREG;
#if !defined(_WIN32)
    mask |= S_IFLNK;
#endif
    ...
    void ReportProgress(LinePrinter& lp, const std::string& file, uint64_t file_copied_bytes,
        ...
        if (bytes_expected != 0 && bytes_transferred <= bytes_expected) {
            ...
            if (overall_percentage <= 100) {
                _snprintf(overall_percentage_str, sizeof(overall_percentage_str), "%d%%",
                         overall_percentage);
            }
        }
...
static int set_time_and_mode(const std::string& lpath, time_t time,
                             unsigned int mode) {
    struct _utimbuf times = { time, time };
	int r1 = _utime(lpath.c_str(), &times);

    /* use umask for permissions */
    int mask = umask(0000);
    umask(mask);
    int r2 = _chmod(lpath.c_str(), mode & ~mask);
